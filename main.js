...
class Event {
    ...
    ticketPrice;
    city;
    constructor(title, date, ticketStock, city, ticketPrice) {
...
class Ticket {
    ...
    userId;
    quantity;
    constructor(eventId, userId, quantity) {
...
class onlineTicketSystem {
    ...
    tickets = [];
    loggedInUser = null;
    adminCredentials = { email: 'baselhussain4@gmail.com', password: 'basel' };
    async start() {
    ...
    async signUp() {
        ...
        if (userDetails.email === this.adminCredentials.email) {
            ...
            if (existingUser) {
                ...
                const newUser = new User(userDetails.name, userDetails.email, userDetails.password, userDetails.admin);
    ...
    async login() {
        ...
        if (loginDetails.email === this.adminCredentials.email && loginDetails.password === this.adminCredentials.password) {
            ...
            const user = this.user.find(user => user.email === loginDetails.email && user.password === loginDetails.password);
            if (user) {
